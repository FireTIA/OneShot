...
from OS_Files.db import USB_MAP, PCI_MAP, Ubiquiti_OUI, MikroTik_OUI, Cisco_OUI
from OS_Files.settings import OS_TARGET, CUSTOM_PATH_FOLDER, HIDE_PASSWORD_AP, HIDE_PIN_AP, HIDE_MAC_AP, MACCHANGER_OUTPUT_MODE, CHECK_CORP_AP_ACTION
import re
import shutil

init(autoreset=True)

Module_prefix = f"{Fore.LIGHTMAGENTA_EX}[{Fore.LIGHTBLUE_EX}Wlan{Fore.LIGHTGREEN_EX}X{Fore.CYAN}-Termux{Fore.LIGHTMAGENTA_EX}]{Fore.RESET}"
...
if shutil.which("macchanger") is None:
    print(f"{Fore.YELLOW}[{Fore.LIGHTRED_EX}!{Fore.YELLOW}]{Fore.RESET} {Module_prefix} macchanger не установлен! (pkg install macchanger)")
    exit()
...
#
# >> Переключатель Wifi-интерфейса Android'а
#


def WLANX__toggle_android_wifi(enable=True):
    """
    Включение/выключение WiFi на Android
    enable=True  - Включение
    enable=False - Выключение
    """
    state = "enable" if enable else "disable"
    cmd = f"svc wifi {state}"
    print_process(f"{Module_prefix} Выполняет переключение Wifi на Android")
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    
    if result.returncode == 0:
        action = "Включил" if enable else "Выключил"
        print_info(f"{Module_prefix} {action} Wifi на Android")
    else:
        print_info(f"{Module_prefix} Ошибка при переключении Wifi (код: {result.returncode})")
    
    print_out_cmd(f"{Module_prefix} Вывод 'toggle_android_wifi': \n {result} \n \n \n ")
...
def WlanX__Mac_Change(iface, type_change):
    if type_change in [False, "False", "false", "f"]:
...
def WlanX__kill_conflicts(iface: str):
    ...
    wpa_supplicant, dhclient, hostapd, Wifi-интерфейс Android'а.
    """
    ...
    # Пытаемся остановить Wifi-интерфейс Android'а
    try:
        subprocess.run(
            ['svc', 'wifi', 'disabled'],
            stdout=subprocess.DEVNULL,
...
def WlanX__Check_Corp_AP(count_retry_check: int, timeout_check: int, iface: str):
    ...
    preset_action = CHECK_CORP_AP_ACTION
