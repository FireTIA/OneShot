...
#(НЕ МЕНЯТЬ, МЕНЯТЬ ТОЛЬКО ДЛЯ ОТЛАДКИ!)
nm_was_active = None          # изначальное состояние NM (True/False/None)
nm_stopped_by_script = False  # флаг: мы реально его останавливали
#--- Временные переменные ---<
...
class Companion:
    ...
    def __init_wpa_supplicant(self):
        print(f'{Fore.GREEN}[{Fore.LIGHTCYAN_EX}*{Fore.GREEN}]{Fore.RESET} Running {Fore.YELLOW}wpa_supplicant{Fore.RESET}…')
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def __wps_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, verbose=None):
        ...
        if pbc_mode:
            if bssid:
                print(f"{Fore.GREEN}[{Fore.LIGHTCYAN_EX}*{Fore.GREEN}]{Fore.RESET} Запуск соединения по кнопке WPS с точкой доступа {Fore.LIGHTYELLOW_EX}{bssid}{Fore.RESET}…")
                cmd = f'WPS_PBC {bssid}'
            else:
...
class WiFiScanner:
    ...
    # > FireSoft
    def _kill_conflicts(self, iface: str):
        """
        Завершаем процессы, которые держат Wi-Fi-интерфейс:
        wpa_supplicant, dhclient, NetworkManager-DHCP, hostapd
        """
        conflict_names = ('wpa_supplicant', 'dhclient', 'hostapd')
        for name in conflict_names:
            subprocess.run(['pkill', '-9', '-f', name], stdout=subprocess.DEVNULL)

        if is_nm_active():                          # был запущен
            subprocess.run(['systemctl', 'stop', 'NetworkManager'], stdout=subprocess.DEVNULL)
            global nm_stopped_by_script
            nm_stopped_by_script = True
        # приводим интерфейс в up, если он down после килла
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        for line in lines:
            if line.startswith('command failed:'):
                ...
                # Определяем именно ошибку -16
                if '(-16)' in line or 'Device or resource busy' in line:
                    ans = input(
                        f"{Fore.CYAN}[?]{Fore.RESET} Интерфейс занят другим процессом. "
                        "Завершить конфликтующие процессы и повторить попытку? [y/N] "
                    ).lower()

                    if ans == 'y':
                        self._kill_conflicts(self.interface)
                        print(f"{Fore.GREEN}[*]{Fore.RESET} Повтор сканирования…\n")
                        return self.iw_scanner()           # ← рекурсивно перезапускаем скан
                    else:
                        print(f"{Fore.LIGHTBLUE_EX}Выход по запросу пользователя.")
                        sys.exit(0)
    ...
    def prompt_network(self) -> str:
        ...
        while 1:
            try:
                networkNo = input('Выберите цель (Enter обновление списка): ')
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            # если мы сами гасили NM и он изначально работал — поднимаем обратно
            if nm_was_active and nm_stopped_by_script:
                subprocess.run(['systemctl', 'start', 'NetworkManager'])
                print(f"{Fore.GREEN}[*]{Fore.RESET} NetworkManager восстановлен")
