...
def WlanX__Mac_Change(iface, type_change):
    ...
    if mode == "raw":
        ...
        if not info["current_mac"] and not info["new_mac"]:
            ...
            current_vendor = f"({info['current_name']})" if info["current_name"] else ""
            permanent_vendor = f"({info['permanent_name']})" if info["permanent_name"] else ""
            new_vendor = f"({info['new_name']})" if info["new_name"] else ""

            print_info(f"{Module_prefix} > [Macchanger] Current  : {Fore.CYAN}{info['current_mac'] or 'N/A'}{Fore.RESET} {current_vendor}")
            print_info(f"{Module_prefix} > [Macchanger] Permanent: {Fore.CYAN}{info['permanent_mac'] or 'N/A'}{Fore.RESET} {permanent_vendor}")
            print_info(f"{Module_prefix} > [Macchanger] New      : {Fore.LIGHTGREEN_EX}{info['new_mac'] or 'N/A'}{Fore.RESET} {new_vendor}")
...
def WlanX__set_tx_power(iface: str, tx_power_dbm: int) -> bool:
    """
    Устанавливает TX Power для интерфейса в dBm.
    Возвращает True при успехе, иначе False.
    """
    if tx_power_dbm < 1 or tx_power_dbm > 30:
        print_info(
            f"{Module_prefix} > [TX-Power]: Недопустимое значение {tx_power_dbm}. "
            f"Ожидается целое число в диапазоне 1-30 dBm."
        )
        return False

    tx_power_mbm = tx_power_dbm * 100

    current_before = WlanX__Adapter_info_get("tx-power-db", iface)
    errors = []

    commands_to_try = [
        ['sudo', 'iw', 'dev', iface, 'set', 'txpower', 'fixed', str(tx_power_mbm)],
        ['sudo', 'iwconfig', iface, 'txpower', str(tx_power_dbm)],
    ]
    ...
    for cmd in commands_to_try:
        try:
            ...
            err = (e.stderr or '').strip() or 'Неизвестная ошибка'
            errors.append(f"{' '.join(cmd)} -> {err}")
            continue

        time.sleep(0.2)
        current_after = WlanX__Adapter_info_get("tx-power-db", iface)

        try:
            current_after_num = float(current_after) if current_after is not None else None
        except ValueError:
            current_after_num = None

        if current_after_num is not None and abs(current_after_num - tx_power_dbm) < 0.5:
            print_process(
                f"{Module_prefix} > [TX-Power]: Для {Fore.LIGHTBLUE_EX}{iface}{Fore.RESET} "
                f"успешно установлена мощность {Fore.LIGHTCYAN_EX}{tx_power_dbm} dBm{Fore.RESET}."
            )
            return True

        errors.append(
            f"{' '.join(cmd)} -> команда выполнилась, но фактический Tx-Power остался "
            f"{current_after if current_after is not None else 'неизвестным'} dBm"
        )

    print_info(
        f"{Module_prefix} > [TX-Power]: Не удалось подтвердить установку "
        f"{Fore.LIGHTCYAN_EX}{tx_power_dbm} dBm{Fore.RESET} для "
        f"{Fore.LIGHTBLUE_EX}{iface}{Fore.RESET}.\n"
        f"Было: {current_before if current_before is not None else 'N/A'} dBm\n"
        f"Стало: {WlanX__Adapter_info_get('tx-power-db', iface) or 'N/A'} dBm"
    )
    if errors:
        print_info(f"{Module_prefix} > [TX-Power]: Детали попыток:\n- " + "\n- ".join(errors))
    return False
...
def WlanX__Check_Corp_AP(count_retry_check: int, timeout_check: int, iface: str):
    ...
    preset_action = settings.get("Check_Corp_AP_Action")
    preset_tx_value = settings.get("Check_Corp_AP_lTX_Value")
    preset_recheck = _settings_bool(settings.get("Check_Corp_AP_Recheck_After_lTX"))

    # Спрашиваем, что делать дальше.
    while True:
        if preset_action is None or str(preset_action).strip().lower() in ("", "none", "null"):
            ans = input(
                f"\n{Module_prefix} > [Check-CorpAP]: Обнаружены возможные корпоративные/управляемые AP.\n"
                f"Продолжить выполнение скрипта? \n"
                f"|> lTX - Занизить TX Power (Зависит от драйвера и чипсета)\n"
                f"|> Yes - Да оставить без изменений\n"
                f"|> No - Нет завершить скрипт\n"
                f">> "
            ).strip().lower()
        else:
            ans = str(preset_action).strip().lower()
            print_info(
                f"{Module_prefix} > [Check-CorpAP]: Применяю действие из настроек: "
                f"{Fore.LIGHTCYAN_EX}{ans}{Fore.RESET}"
            )

        if ans in ("y", "yes", "д", "да"):
            print_info(f"{Module_prefix} > [Check-CorpAP]: Продолжаем работу по запросу пользователя.\n")
            return
        if ans in ("ltx", "tx", "l", "lt", "lx"):
            if preset_tx_value is None or str(preset_tx_value).strip().lower() in ("", "none", "null"):
                tx_value_raw = input(
                    f"{Module_prefix} > [TX-Power]: Введите целое значение TX Power в dBm "
                    f"(пример: 5, 10, 20, 30): "
                ).strip()
            else:
                tx_value_raw = str(preset_tx_value).strip()
                print_info(
                    f"{Module_prefix} > [TX-Power]: Применяю значение из настроек: "
                    f"{Fore.LIGHTCYAN_EX}{tx_value_raw} dBm{Fore.RESET}"
                )

            try:
                tx_value_dbm = int(tx_value_raw)
            except ValueError:
                print("!Введите целое число для TX Power!")
                if preset_action is not None:
                    return
                continue

            if not WlanX__set_tx_power(iface, tx_value_dbm):
                if preset_action is not None:
                    return
                continue

            if preset_recheck is None:
                recheck_answer = input(
                    f"{Module_prefix} > [Check-CorpAP]: Повторно проверить эфир на корпоративные AP? [Y/n]: "
                ).strip().lower()
                recheck_enabled = recheck_answer not in ("n", "no", "н", "нет")
            else:
                recheck_enabled = preset_recheck
                print_info(
                    f"{Module_prefix} > [Check-CorpAP]: Повторная проверка после lTX из настроек: "
                    f"{Fore.LIGHTCYAN_EX}{recheck_enabled}{Fore.RESET}"
                )

            if recheck_enabled:
                print_process(f"{Module_prefix} > [Check-CorpAP]: Запускаю повторную проверку после изменения TX Power.\n")
                return WlanX__Check_Corp_AP(count_retry_check, timeout_check, iface)

            print_info(f"{Module_prefix} > [Check-CorpAP]: Продолжаем работу после изменения TX Power.\n")
            return
        if ans in ("n", "no", "н", "нет", ""):
            print_info(f"{Module_prefix} > [Check-CorpAP]: Завершение работы по запросу пользователя.\n")
            sys.exit(0)
        print("!Повторите ввод (lTX, y/yes/д/да или n/no/н/нет)!")
